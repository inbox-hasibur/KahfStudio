...
2. **Complete Raw Supabase DDL SQL Schema** (Authoritative Supabase PostgreSQL Schema DDL).
...
#### 1. `profiles`
Stores user profile details linked directly to Supabase Auth (`auth.users`).
...
- Fields:
  ...
  - `image_url` (`text`): Main cover banner image URL (extracted via OpenGraph/schema).
  - `category` (`text`, default `'General'`): News topic (`'General'`, `'Sports'`, `'Politics'`, etc.).
  - `audio_bn_summary` (`text`): Hosted Cloudinary WAV link for Bengali summary audio.
  - `audio_bn_full` (`text`): Cloudinary link for Bengali full article audio.
  - `audio_en_summary` / `audio_en_full` (`text`): Cloudinary links for English audio.
  - `importance_score` (`integer`, default `50`): AI rating (1-100) determining news rank.
  - `country` (`text`, default `'BD'`): Target country code (`'BD'`, `'US'`, `'GLOBAL'`).
...
- Fields:
  - `archive_type` (`text`, required): Type of archive (e.g., `'daily_bulletin'`).
  - `title` (`text`, required): Episode title (e.g., `'দৈনিক সংবাদ বুলেটিন'`).
  - `audio_url` (`text`, required): Hosted Cloudinary audio URL.
