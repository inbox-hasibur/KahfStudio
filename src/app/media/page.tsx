...
import { Play, Pause, Music, Video, SkipBack, SkipForward, Rewind, FastForward, Volume2, Settings, Maximize, Lock } from "lucide-react";
                    ...
                    <Card className="bg-card/50">
                        <CardHeader className="pb-3">
                            <CardTitle className="text-lg flex items-center justify-between">
                                <span className="flex items-center gap-2"><Music className="w-4 h-4 text-primary" /> হালাল মোড</span>
                                {!isPremium && <Lock className="w-4 h-4 text-muted-foreground" />}
                            </CardTitle>
                        </CardHeader>
                        <CardContent className="space-y-3">
                            <div className={`flex items-center justify-between p-3 rounded-lg border ${isPremium ? 'bg-muted border-border' : 'bg-muted/50 border-border/50 opacity-60'}`}>
                                <div className="text-sm font-medium">মিউজিক ছাড়া (এআই)</div>
                                <Button variant="outline" size="sm" disabled={!isPremium}>
                                    {isPremium ? "চালু করুন" : "Upgrade to Premium (Halal)"}
                                </Button>
                            </div>
